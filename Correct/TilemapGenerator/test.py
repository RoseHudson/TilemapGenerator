...
from final import create_enemy
...
class TestCreateEnemy(unittest.TestCase):
    ...
    def test_1_1_dict_structure(self):
        enemy = create_enemy()
        self.assertIsInstance(enemy, dict)
        self.assertIn('type', enemy.keys())
        self.assertIn('level', enemy.keys())
    ...
    def test_1_5_level_randomness(self):
        for enemy, (min_lvl, max_lvl) in self.enemies_dict.items():
            levels_seen = set()
            for _ in range(200):
                result = create_enemy(type=enemy)
    ...
    def test_5_1_min_level_only(self):
        with self.assertRaises(ValueError):
            enemy = create_enemy(None, 3, None)

    def test_5_2_max_level_only(self):
        with self.assertRaises(ValueError):
            enemy = create_enemy(None, None, 100)
    
    def test_5_3_type_and_only_one_bound(self):
        with self.assertRaises(ValueError):
            enemy = create_enemy("vampire", None, 100)
    ...
    #     with self.assertRaises(ValueError):
    #         enemy = create_enemy(None, 50, 20)

    # def test_5_6_min_too_small(self):
    #     with self.assertRaises(ValueError):
    #         enemy = create_enemy(None, -5, 20)

    # def test_5_7_max_too_large(self):
    #     with self.assertRaises(ValueError):
    #         enemy = create_enemy(None, 0, 150)

    # def test_5_8_case_sensitivity_in_type(self):
    #     with self.assertRaises(ValueError):
    #         enemy = create_enemy("Vampire")
